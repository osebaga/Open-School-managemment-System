...
class Applicant extends CI_Controller
{
    ...
    function is_applicant_pay()
    {
        ...
        $amount = $this->applicant_model->get_paid_amount($this->APPLICANT->id,1,$ActiveYear);        $amount_required = APPLICATION_FEE;
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (count($array1) >= 1) {
                ...
                $array_data = array(
                    ...
                    'choice3' => trim($this->input->post('choice3')),
                    'round'=>$round,
                    'application_campus' =>$this->input->post('application_campus'),
                    'createdby' => $current_user->id,
                    'createdon' => date('Y-m-d H:i:s'),
                    'applicant_id' => $this->APPLICANT->id,
                    'AYear' => $ayear
