...
$sheet->setCellValue('B3', 'SELECTED APPLICANT LIST REPORT - ' . $this->common_model->get_academic_year()->row()->AYear);
...
$column++;
$sheet->setCellValue($column . $rows, 'Full Name');
$column++;
...
for($i=0;$i<count($applicants);$i++) {
  ...
  $fullname = $this->db->get_where('application',array('id'=>$applicant_id))->row();
    ...
    // $column++;
    $sheet->setCellValue($column . $rows, $applicants[$i]['MobileNumber']);
    $column++;
    $sheet->setCellValue($column . $rows, $applicants[$i]['EmailAddress']);
    $column++;
    $sheet->setCellValue($column . $rows, $applicants[$i]['AdmissionStatus']);
    $column++;
...
$this->excel->Output("ADMITTED APPLICANT LIST");
exit;
?>
