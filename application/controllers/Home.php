...
class Home extends CI_Controller
{
    ...
    function git_pull(){
        $output=null;
        $retval=null;
        exec("/bin/bash /var/lib/git-command/oas-osmis/git-pull.sh",$output,$retval);
        echo "Returned with status $retval and output:\n";
        print_r($output);
    }
    function registration_start()
    { 
        // $this->data['regions'] = $this->db->query("select * from regions")->result();
    ...
    {
        $this->data['content'] = 'home/center_registration';
        $this->load->view('public_template', $this->data);
    }

    function center_application()
    {
        if (isset($_GET) && isset($_GET['type']) && isset($_GET['premises'])) {
            $this->data['type'] = $application_type = $_GET['type'];
            $this->data['premises'] = $premises = $_GET['premises'];
            ...
            $this->form_validation->set_rules('education_level', "Owner's Education level", 'required');
            $this->form_validation->set_rules('cordinator', 'Center Cordinator', 'required');
            $this->form_validation->set_rules('nationality', 'Nationality', 'required');
            $this->form_validation->set_rules('TIN', 'Taxpayer Identification Number', 'required|is_unique[application.TIN]');          
            ...
            $this->form_validation->set_rules('centreid', 'Application Type', 'required');
            $this->form_validation->set_rules('premises', 'Center Premises', 'required');
            $this->form_validation->set_rules('idnumber', 'NIDA', 'required');
            ...
            if ($this->form_validation->run() == true && ($required == true)) {
                ...
                if ($row_year) {
                    $array_data = array(
                        ...
                        'CenterName' => ucfirst(strtolower(trim($this->input->post('centername')))),
                        'CenterOwner' => ucfirst(strtolower(trim($this->input->post('centerowner')))),
                        'CenterCordinator	' => ucfirst(strtolower(trim($this->input->post('cordinator')))),
                        'national_identification_number	' => trim($this->input->post('idnumber')),
                        'Email' => strtolower(trim($this->input->post('email'))),
                        'TIN' => trim($this->input->post('TIN')),
                        'OwnerProfession' => trim($this->input->post('education_level')),
                        'Nationality' => trim($this->input->post('nationality')),
                        'residence_country' => trim($this->input->post('residence_country')),
                        'region'=>get_value('regions',$this->input->post('region'),'id'),
                        'district'=>get_value('districts',$this->input->post('district'),'id'),
                        // 'primary_school' =>trim($this->input->post('primary_school')),
                        'createdon' => date('Y-m-d H:i:s'),
                        // 'CenterRegNo'=>trim($this->input->post('centreid')),
                        'application_category'=>'Center',
                    ...
                    $register = $this->applicant_model->new_center($array_data, trim($this->input->post('password')));
                    if ($register) {
                        ...
                        redirect('center_contact', 'refresh');
                    } else {
...
//                }
            ...
            $this->data['content'] = 'home/center_application';
            $this->load->view('public_template', $this->data);
        } else {
            redirect('center_application', 'refresh');
        }
    }
    ...
    function application_start()
    {
        if (isset($_GET) && isset($_GET['type']) && isset($_GET['entry'])) {
            ...
            $this->form_validation->set_rules('region', 'Region of Residence', 'required');
            $this->form_validation->set_rules('district', 'District of Residence', 'required');
            $this->form_validation->set_rules('centreid', 'Center', 'required');
            $this->form_validation->set_rules('entry_mod', 'Entry Mode', 'required');
            ...
            if ($this->form_validation->run() == true && ($required == true)) {
                ...
                if ($row_year) {
                    $array_data = array(
                        ...
                        'Semester' => $row_year->semester,
                        'application_type' => $application_type,
                        'entry_category' => trim($this->input->post('entry_mod')),
                        // 'duration' => programme_duration($application_type),
                        ...
                        'createdon' => date('Y-m-d H:i:s'),
                        'CenterRegNo'=>trim($this->input->post('centreid')),
                        'application_category'=>'Applicant',
                    );
...
//        $captcha_num = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890abcdefghijklmnopqrstuvwxyz';
    ...
    function loadEducationData()
    {
        ...
        if ($action == 'o-level') {
            ...
            $where_array = array('index_number' => $o_index_number, 'api_status' => 1, 'certificate' => 1);
            ...
            if ($education_row) {
                ...
                $full_name = trim($responsedata->particulars->first_name) . trim($responsedata->particulars->last_name);
                ...
                if ($check_equivalent == "EQ") {
                    ...
                    $index_number = $index_number_tmp[0];
                    $completed_year = $index_number_tmp[1];
                } else {
                    $index_number_tmp = explode('/', $o_index_number);
                ...
                if ($response) {
                    ...
                    if ($responsedata->status->code == 1) {
                        ...
                        $data = array(
                            ...
                            'division' => $responsedata->results->division,
                            'country' => 220,
                            'division_point' => $responsedata->results->points,
                            'api_status' => 1,
                            'createdby' => 0,
                            'comment' => 'Success',
                            'createdon' => date('Y-m-d H:i:s'),
            ...
            $where_array = array('index_number' => $a_index_number, 'api_status' => 1, 'certificate' => 2);
            $education_row = $this->db->where($where_array)->get('application_education_authority')->row();
                ...
                if ($response) {
                    ...
                    if ($responsedata->status->code == 1) {
                        ...
                        $data = array(
                            ...
                            'center_number' => $responsedata->particulars->center_number,
                            'school' => $responsedata->particulars->center_name,
                            // 'division' => $responsedata->results->division->division,  //old api
                            'division' => $responsedata->results->division,
                            'country' => 220,
                            'division_point' => $responsedata->results->points,
                            'api_status' => 1,
            ...
            if ($education_row) {
                $response = $education_row->response;
                $responsedata = json_decode($response);
                $full_name = trim($responsedata->params[0]->firstname) . trim($responsedata->params[0]->surname);
...
//        $user=current_user();
    ...
    function student_create_invoice($id=null)
    {
        ...
        if (!is_null($id)) {
            ...
            $check_student = $this->db->query("select * from application where RegNo='" . $id . "' and RegNo<>''")->row();
            if ($check_student->RegNo) {
                $this->data['s_name'] = $check_student->LastName;
                $this->data['m_name'] = $check_student->MiddleName;
                $this->data['f_name'] = $check_student->FirstName;
                $this->data['email'] = $check_student->Email;
                $this->data['phone'] = $check_student->Mobile1;
                $nta_level=$check_student->CenterRegNo;
                $fee_structure = $this->db->query("select * from fee_structure where ntlevel_value='" . $nta_level. "'  order by ntlevel_value DESC")->result();
        ...
        if($this->input->post('loadsheet')) {
            ...
            if ($this->input->post('invoice_type') == 4) {
                ...
                if ($this->input->post('invoice_type') != 3) {
                    ...
                    $this->form_validation->set_rules('institutionname', 'Center Name', 'required');
                }
            ...
            if ($this->form_validation->run() == true) {
                ...
                if ($this->input->post('invoice_type') == 3) {
                    $centerregno = trim($this->input->post('institutionname'));
                    $reg_no=$centerregno;
                    $center_details=$this->db->query("select * from Center where CenterRegNo='".$centerregno."'")->row();
                    $firstname=$center_details->CenterName;
                }elseif($this->input->post('invoice_type') == 1)
                {
                    $student_details=$this->db->query("select * from application where RegNo='".$reg_no."'")->row();
                    $centerregno=$student_details->CenterRegNo;
                }
...
//        }
    ...
    function LoadStudentDetailsByID()
    {
        ...
        $select="select * from application  where RegNo='".$regno."'";
        $students_list=$this->db->query($select)->row();
        if($students_list->RegNo)
        {

            $name=$students_list->LastName .', '.$students_list->FirstName .' '.$students_list->MiddleName;
            $mobile=$students_list->Mobile1;
