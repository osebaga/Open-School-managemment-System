...
if($gpa_required > 0) {
    ...
    $this->db->where(array('certificate' => 4, 'applicant_id' => $applicant_id,'api_status'=>1));
    ...
    if ($DIPLOMA_CERTIFICATE) {
        $DIPLOMA_LEVEL_HOLD[$DIPLOMA_CERTIFICATE->avn][] = "Category = ".$DIPLOMA_CERTIFICATE->programme_category;
        ...
        if ($DIPLOMA_CERTIFICATE->division >= $gpa_required) {
            ...
            //     }else{
            //         $is_eligible = 0;
            //         $remark = "Diploma tittle do not contain any of".$setting_info->keyword1.' as keyword';
            //     }
            // }else {
                $GPA = $DIPLOMA_CERTIFICATE->division;
                $is_eligible = $diploma_level_status = 1;
        ...
        $is_eligible = 0;
        $remark = "No diploma Information found";
    }
...
if($o_level_status && $a_level_status) {
    if ($is_eligible) {
    ...
    $is_eligible = 0;
    $diploma_level_status = 0;
    $diploma_level_remark = $a_level_remark;
}
