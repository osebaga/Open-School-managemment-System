...
if($gpa_required > 0) {
    ...
    $this->db->where(array('certificate' => 3, 'applicant_id' => $applicant_id));
    ...
    if ($DIPLOMA_CERTIFICATE) {
        ...
        if ($DIPLOMA_CERTIFICATE->division >= $gpa_required) {
            if($setting_info->keyword1 <> '') {
                ...
                if($contain_keyword > 0){
                    ...
                    $is_eligible = 0;
                    $remark = "Certificate tittle do not contain any of".$setting_info->keyword1.' as keyword';
                }
            }else {
                $GPA = $DIPLOMA_CERTIFICATE->division;
                $is_eligible = $diploma_level_status = 1;
        ...
        $is_eligible = 0;
        $remark = "No Certificate Information found";
    }
