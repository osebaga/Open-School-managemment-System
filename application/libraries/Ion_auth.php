	...
	public function forgotten_password($identity)    //changed $email to $identity
	{
      ...
      $user = $this->where($identifier, $identity)->where('active', 1)->users()->row();  // changed to get_user_by_identity from email
			...
			if ($user)
			{
                ...
                $email_data['title'] = 'Password Reset';
                $email_data['salutation'] = 'Dear ' . $user_info->firstname . ' ' . $user_info->lastname;
                $email_data['content'] = "Password Reset Instructions <br/><br/>
Someone requested that the password be reset for the following account: <br/>
Username: ".$user_info->username.'<br/>
If this was a mistake, just ignore this email and nothing will happen. To reset your password, visit the following address: <br/>
<a href="'.site_url('auth/reset_password/'.$user_info->forgotten_password_code).'">Click here to reset your password</a> <br/>';

                $email_message = $this->load->view('email_template/template', $email_data, true);

                $send = $this->mail->send_email('PASSWORD RESET', $email_message, array($user_info->email));
