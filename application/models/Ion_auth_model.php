...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login,campus_id')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description, '.$this->tables['groups'].'.module_id')
	...
	**/
	...
	function user_object($user_id){
		$sql = "SELECT u.*, g.id as group_id,g.description as group_name FROM users as u INNER JOIN
                 users_groups ON users_groups.user_id=u.id INNER JOIN groups as g ON g.id=users_groups.group_id WHERE u.id=$user_id";
